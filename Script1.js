...
class Box {
	...
	setOrientation() {
		...
		console.log("rotated box from ("+x+","+y+","+z+") to ("+this.width+","+this.length+","+this.height+").")
	}
...
while (unorderedContents.length > 0) {
	console.log("running " + unorderedContents.length + " times");
	maxArea = 0
	largestElement = 0
...
for (c in contents) {
	for (b in bins) {
		if (Math.max(bins[b].height, bins[b].width, bins[b].length) < Math.max(contents[c].height, contents[c].width, contents[c].length)) {
			tooSmall += 1;
		}
...
function fillSpace(cont, vol) {
			...
			if (x.isUpright) {
				...
				if (fitItem(x, vol) == null) {
					...
					if (fitItem(x, vol) == null) {
						...
						fittingItems.push(x);
						cont.splice(0, 1);
						console.log("fittingItems is " + fittingItems.length + ".");
					}
				} else {
					//if space is lowered then the item fits
					console.log("added item to fittingItems");
	...
	console.log(returnArray[0].length, returnArray[1].length);
	return returnArray;
}
...
function selectBin() {
	while (bins.length > 0) {
		...
		console.log("trying first stack");
		...
		//console.log("leftOverFloor is " + leftOverFloor.length);
			...
			if (fitFloor.length == contents.length) {
				console.log("this bin will fit in one layer, bin " + bins[0].length + ", " + bins[0].width + ", " + bins[0].height + ".");
				return;
			}


		console.log("reached second stack")
		leftOverFirst = leftOverFloor;

		for (f in fitFloor) {
			console.log("trying second stack, item" + f);
			...
			if (fitFloor.length + fitFirst.length == contents.length) {
				console.log("this bin will fit all items in two layers, bin " + bins[0] + ".");
				return;
		}
		...
		console.log("This bin will fit " + fitFloor.length +" in the first layer, "+ fitFirst.length+" in the second layer, " /*" + fitSecond.length +",*/ + " with " + /*leftOverSecond*/ leftOverFirst.length + " items left over.");
